...
let private update () =
    ...
    | Playing ->
        if pressed "KeyP" then app.Paused <- not app.Paused
...
let private render () =
    ...
    | Playing ->
        renderFrame ctx gs viewW viewH false
        if app.Paused then drawPauseOverlay ctx viewW viewH
    ...
    | LeaguePlaying ->
        renderFrame ctx gs viewW viewH true
        if app.Paused then drawPauseOverlay ctx viewW viewH
